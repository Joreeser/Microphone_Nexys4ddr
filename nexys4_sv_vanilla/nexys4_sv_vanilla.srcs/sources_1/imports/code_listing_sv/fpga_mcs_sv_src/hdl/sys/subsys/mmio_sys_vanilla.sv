...
(
   ...
   (.clk(clk),
    ...
    // slot 14: microphone
    microphone_wrapper mic_slot14
    (.clk(clk),
     .reset(reset),
     .cs(cs_array[`S14_MIC]),
     .read(mem_rd_array[`S14_MIC]),
     .write(mem_wr_array[`S14_MIC]),
     .addr(reg_addr_array[`S14_MIC]),
     .rd_data(rd_data_array[`S14_MIC]),
     .wr_data(wr_data_array[`S14_MIC]),
     .M_DATA(M_DATA),
     .M_LRSEL(M_LRSEL),
     .M_CLK(M_CLK)
     );
    ...
    // slot 15: audio
    audio_wrapper audio_slot15
    (.clk(clk),
     .reset(reset),
     .cs(cs_array[`S15_AUDIO]),
     .read(mem_rd_array[`S15_AUDIO]),
     .write(mem_wr_array[`S15_AUDIO]),
     .addr(reg_addr_array[`S15_AUDIO]),
     .rd_data(rd_data_array[`S15_AUDIO]),
     .wr_data(wr_data_array[`S15_AUDIO]),
     .audio_pdm(audio_pdm),
     .audio_on(audio_on)
     );
   ...
   generate
      genvar i;
      for (i=4; i<14; i=i+1) begin:  unused_slot_gen
   ...
   generate
         genvar j;
         for (j=16; j<64; j=j+1) begin:  unused_slot_gen2
            assign rd_data_array[j] = 32'hffffffff;
         end
      endgenerate
endmodule
